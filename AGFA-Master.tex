% !TEX TS-program = pdflatexmk
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Das Masterefile: AGFA-Master.tex  
%% Stand:	2024/07/19 
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\documentclass[%	 -- siehe KOMA-Script 
	,toc		= bib 		 
	,parskip	= half-		 
	,headings	= normal	 		 	 		 
	,numbers	= noenddot		
	,BCOR 		= 12mm
%	,twoside 	= true		%% falls 2-seitig % wegmachen
	,version 	= last
 	,titlepage	= true   
	,ngerman	
%	,english	
% 	,review					% für das Korrekturlesen % wegmachen	 
	]{scrartcl}			 

%% -- Nutze die AGFA-Pakete
%% -- Für das Literaturverzechnis kann genutzt werden
%% -- numeric/alphabetic/philosophy + doiurl. numeric ist der Default, wenn
%% -- nichts anderes angegeben wird. Ansosnten siehe ReadMe-Dokumentationen
%% --
\usepackage[%
%	, numeric			% Default
%	, alphabetic		% [GR80]
%	, philosophy		% wie im Buch "Banach Lattices of Positive Operators"
  	, urldoi			% Wer doi im Titel hinterlegt haben will, sonst auskommentieren
%	, libertinus		% font libertinus
%	, lmodern			% font lmodern, das Original von Knuth
%	, thmframed		% wer es gerahmt haben will
	]{./preamble/agfa-art}
	
%% -- Literatur-DB durch eigene ersetzen 
%% --
\addbibresource{./bib/agfa-bib.bib}		% agfa-bib.bib -> abcd-bib.bib	

\ExecuteBibliographyOptions{%
	,backref		= true	% Auf was ist verwiesen; Rest muss weg
  	,url			= true	% false: URL wird nicht angezeigt 
  	,doi			= false	% true: URLDOI wird separat angezeigt
	,eprint			= false	% true: wird separat angezeigt
	,maxcitenames  	= 1 	% viele Autoren -> nur der erste erscheint  
    ,maxbibnames   	= 10 	% im Verzeichnis tauchen alle auf; Auskommentieren für philosphy 
	}

%% -- Links farbig
%% --

\hypersetup{
	,colorlinks	= true    			%   onlineversion true                                                           
	,urlcolor	= blue       		%                                                              
	,citecolor	= blue      		%                                                          
	,linkcolor	= blue			 	% oder black
%  	,hidelinks 						% Vor dem Druck % entfernen
	}
	

%% == Ab hier eigene Angaben ==
	
%% -- Was soll aktuell bearbeitet werden
%% -- Mittels % entsprechend steuern
%% --

\includeonly{%
	./content/AGFA-Einleitung,
	./content/AGFA-Section-1,
	./content/AGFA-Section-2,
	}


%% -- Ausfüllen aber für die finale Version mit % auskommentieren
%% --
\KOMAoptions{footsepline}
\lofoot{Name}
\cofoot{Stand der Arbeit:}
\rofoot{\today}

%% -- Paket comment
%% -- siehe Literaturverzeichnis
%% --

\usepackage{comment}

%% -- Start
%% --
\begin{document}

%% %%%%%%%%%%%%%%%%%%%%%%%%%
%% Frontmatter
%% %%%%%%%%%%%%%%%%%%%%%%%%

%% -- Bei einer Arbeit zum Schluss bei \begin{comment} und \end{comment}
%% -- ein % hinzufügen und nicht vergessen, titlepage = true setzen
%% --

%\begin{comment}
%%
\begingroup
	\input{./content/0-AGFA-title}			% Titelseite
	\pagestyle{empty}
	\input{./content/0-AGFA-EigenErkl}		% Eigenständigkeitserklärung
	\input{./content/0-AGFA-thanks}			% Danksagung
	\input{./content/0-AGFA-summary}		% Zusammenfassung
	\tableofcontents 
	\thispagestyle{empty}
\endgroup
%%
%\end{comment}

%% %%%%%%%%%%%%%%%%%%%%%%%%%
%% Mainmatter
%% %%%%%%%%%%%%%%%%%%%%%%%%

\pagenumbering{arabic}
\setcounter{page}{0}
\setcounter{section}{0}

%% -- Die einzelnen Abschnitte werden über \includeonly eingebaut 
%% -- Dies bewirkt einen Seitenvorschub  und man muss zum Schluss
%% -- sehen, was dieser bewirkt hat. Im Zweifel dann das 
%% -- \include in ein \input abändern. Es ist aber bei der Erstellung
%% -- so bequemer, da die Querverweise etc. erhalten bleiben.
%% --

\include{./content/AGFA-Einleitung}		% Die Einleitung=Vorwort
\include{./content/AGFA-Section-1}		% Erster Abschnitt
\include{./content/AGFA-Section-2}		% Zweiter Abschnitt
% \include{name}						% etc; bitte den Aufbau beachten

\cleardoubleoddpage 

%% %%%%%%%%%%%%%%%%%%%%%%%%
%% Backmatter
%% %%%%%%%%%%%%%%%%%%%%%%%%

\thispagestyle{empty}
\pagenumbering{roman}
\setcounter{page}{1}

%% -- Literaturverzeichnis
%%
%\nocite{}		% für Literatur, die nicht zitiert wurde; 
				% wenig sinnvoll dieses zu tun; siehe ReadMe.pdf
				
\RaggedRight		% Kein Blocksatz

%% -- Beispiele für hinterlegte DOI
%% --
\nocite{kahle:2018,zabreiko:1969,comment}
\printbibliography

%% --

\end{document}
